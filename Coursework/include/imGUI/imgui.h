...
public:
    ...
    inline ImVector(const ImVector<BufferType>& src)                     { Size = Capacity = 0; Data = NULL; operator=(src); }
    inline ImVector& operator=(const ImVector<BufferType>& src)          { clear(); resize(src.Size); memcpy(Data, src.Data, (size_t)Size * sizeof(value_type)); return *this; }
    ...
    inline bool         contains(const value_type& v) const             { const BufferType* data = Data;  const BufferType* data_end = Data + Size; while (data < data_end) if (*data++ == v) return true; return false; }
...
template<typename BufferType> void IM_DELETE(BufferType* p)   { if (p) { p->~BufferType(); ImGui::MemFree(p); } }
